...
def main():
    ...
    session_url = 'http://127.0.0.1:3000/api/sessions/'+quote(name,safe='')
    session = request(session_url, headers=waha_headers)
    config = session.get('config') or {}
    from datetime import datetime, timezone
    backup = STATE/('session-before-n8n-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')+'.json')
    if not backup.exists():
        backup.write_text(json.dumps({'name':name,'config':config},indent=2),encoding='utf-8')
    target = 'http://host.docker.internal:5678/webhook/aura-waha-entrada'
    hooks = [hook for hook in config.get('webhooks',[]) if hook.get('url') not in (target,target.replace('/webhook/','/webhook-test/'))]
    ...
    request(session_url,'PUT',{'name':name,'config':config},waha_headers)
    verified = request(session_url,headers=waha_headers)
    ...
    print('Pendente: enviar uma nova mensagem de um contato autorizado e validar a resposta no celular.')
